...
(() => {
  ...
  function init(root){
    ...
    const copyItems = Array.from(copyWrap?.querySelectorAll('.item') || []);
    const pager     = root.querySelector('#TestimonialPager-' + id);
    const pagerItems= Array.from(pager?.querySelectorAll('.pager__item') || []);
    ...
    // Ensure bottom height is stable
    ...
    function setActive(i, instant){
      ...
      copyItems.forEach((el, j) => {
        ...
        if (on) {
          // reset text animations for replay
    ...
    function restartKenBurns(){
      ...
      active?.querySelectorAll('.bg-media img, .bg-media video').forEach(el => {
        el.style.animation = `kb-${id} 10s ease forwards`;
      });
    }

    // Sync on init and change
    setActive(0, true);
    bg.on('init', () => { setActive(realIndex(bg), true); restartKenBurns(); resize(); });
    bg.on('slideChange', () => { setActive(realIndex(bg), false); restartKenBurns(); });
